...
<a onclick="closeFluList()"><i class="fas fa-times" id="flu_close"></i></a>
<script>
    function closeFluList() {
        var chatbox = document.getElementById("flu_list");
        chatbox.style.display = chatbox.style.display === "none" ? "block" : "none";
    }
</script>

<body>
    <p class="text-center fs-1">Flu remedies Lists</p>
    <?php
    include 'database/config.php';

    // Get the total number of rows
    $countSql = "SELECT COUNT(*) as total FROM items WHERE what_for = 'COLD AND FLU RELIEF'"; //replace with what_for
    ...
    $sql = "SELECT code, item_name, type FROM items WHERE what_for = 'COLD AND FLU RELIEF' ORDER BY item_name ASC LIMIT $limit OFFSET $offset"; //replace what_for
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<input type='number' id='gotoPageF' class='form-control' min='1' max='$totalPages' placeholder='Page'>";
        echo "</div>";
        echo "<div class='col-md-1'>";
        echo "<button class='btn btn-danger' id='goButtonF'>Go</button>";
        echo "</div>";
